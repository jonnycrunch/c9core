    ...
    ensureDaemon(function(err) {
        ...
        workerUtil.execAnalysis(
            "bash", // TODO: don't use bash here, better GOPATH handling
            {
                args: [
                    "-c",
                    "GOPATH=/usr/local/go/bin &&~/.c9/gocode/bin/gocode -f=json autocomplete " + getOffset(doc, pos)
                ],
                mode: "stdin",
                json: true,
            },
    ...
    workerUtil.spawn(
        "bash",
        {
            args: [
                ...
                "-c", "mkdir -p ~/.c9/gocode; /usr/local/go/bin/go get -u github.com/mdempsky/gocode && ~/.c9/gocode/bin/gocode"
            ]
        },
        function(err, child) {
